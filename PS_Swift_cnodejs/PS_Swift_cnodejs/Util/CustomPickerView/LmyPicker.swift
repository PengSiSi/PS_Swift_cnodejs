...
class LmyPicker: UIView,UIPickerViewDelegate,UIPickerViewDataSource {
    ...
    init(delegate: PickerDelegate, style: LmyPickerStyle) {
        ...
        switch style {
        case .nomal:
            self.picker = UIPickerView(frame: CGRect(x: 0, y: 44, width: k_ScreenWidth, height: picker_height - 44))
            self.picker.delegate = self
            self.picker.dataSource = self
            self.picker.backgroundColor = UIColor.white
            self.addSubview(self.picker)
        case .date:
            self.datePicker = UIDatePicker(frame: CGRect(x: 0, y: 44, width: k_ScreenWidth, height: picker_height - 44))
